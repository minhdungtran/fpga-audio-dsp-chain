...
async def send_fir_frames(dut, input_frames):
    for left, right in input_frames: 
          await FallingEdge(dut.top_dsp_clk)
          dut.top_fir_in_valid.value = 1
...
async def one_frame(dut):
    ...
    receive_task = cocotb.start_soon(receive_frames(dut, len(input_frames)))
    send_task = cocotb.start_soon(send_fir_frames(dut, input_frames))

    actual_output = await with_timeout(receive_task, 5, "ms")
    await with_timeout(send_task, 5, "ms")
